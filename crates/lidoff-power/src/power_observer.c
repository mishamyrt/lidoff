...
static void powerCallback(void *ref_con, io_service_t service,
...
uint8_t PowerObserverRunLoop(PowerObserverCallback will_sleep,
  ...
  power_root_port = IORegisterForSystemPower(NULL, &power_notify_port,
                                             powerCallback, &power_notifier);
