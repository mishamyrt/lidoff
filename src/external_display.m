...
static const ExternalDisplayBackend *kBackends[] = {NULL, NULL, NULL};
...
static size_t backendCount(void) { return sizeof(kBackends) / sizeof(kBackends[0]); }
...
ExternalDisplayDisableResult ExternalDisplaysDisable(void) {
  ...
  ExternalDisplayDisableResult result = {.ok = YES,
...
ExternalDisplayRestoreResult ExternalDisplaysRestore(void) {
  ensureBackendsInitialized();
  ExternalDisplayRestoreResult result = {.ok = YES, .hadBackups = NO, .restored = 0};
...
ExternalDisplayRestoreResult ExternalDisplaysRestoreFromState(NSDictionary *state) {
  ensureBackendsInitialized();
  ExternalDisplayRestoreResult result = {.ok = YES, .hadBackups = (state != nil), .restored = 0};
