...
BOOL ExternalDisplayGammaHasBackups(void) { return displayBackupCount > 0; }
...
static BOOL ddcSetVCP(io_service_t framebuffer, uint8_t code, uint16_t value) {
  uint8_t sendBuffer[7] = {DDC_HEADER, DDC_LEN_SET_VCP,       DDC_CMD_SET_VCP,
...
static BOOL backupAndZeroGamma(CGDirectDisplayID displayID, DisplayBackup *backupOut) {
  ...
  CGError err =
      CGGetDisplayTransferByTable(displayID, (uint32_t)capacity, red, green, blue, &sampleCount);
...
static BOOL restoreDisplayFromBackup(const DisplayBackup *backup) {
  ...
  if (backup->gammaSampleCount > 0 && backup->gammaRed && backup->gammaGreen && backup->gammaBlue) {
    CGSetDisplayTransferByTable(backup->displayID, backup->gammaSampleCount, backup->gammaRed,
                                backup->gammaGreen, backup->gammaBlue);
  }
...
BOOL ExternalDisplayGammaDisableDisplay(CGDirectDisplayID displayID) {
  DisplayBackup backup = {.displayID = displayID,
                          .brightness = 0,
                          .contrast = 0,
                          .hasBrightness = NO,
                          .hasContrast = NO,
...
static NSDictionary *copyGammaState(void) {
  ...
  for (size_t i = 0; i < displayBackupCount; i++) {
    ...
    if (backup->gammaSampleCount > 0 && backup->gammaRed && backup->gammaGreen &&
        backup->gammaBlue) {
...
static DisplayBackup gammaBackupFromState(NSDictionary *entry, BOOL *validOut) {
  DisplayBackup backup = {.displayID = 0,
                          .brightness = 0,
                          .contrast = 0,
                          .hasBrightness = NO,
                          .hasContrast = NO,
