...
static NSString *generatePlistContent(int threshold) {
  ...
  NSString *realPath = [[NSFileManager defaultManager] destinationOfSymbolicLinkAtPath:execPath
                                                                                 error:nil]
                           ?: execPath;
...
BOOL LaunchAgentInstall(int threshold) {
  ...
  if (![content writeToFile:plistPath atomically:YES encoding:NSUTF8StringEncoding error:&error]) {
