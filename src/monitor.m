...
static MonitorState monitorState = {.savedBrightness = -1.0f,
...
static void persistRecoveryStateLocked(void) {
  RecoveryState state = {.pendingBrightnessRestore = (monitorState.brightnessLowered &&
...
static void handlePartiallyClosedLocked(int angle, NSTimeInterval now) {
  ...
  NSTimeInterval sinceWake = (monitorState.lastWakeAt > 0.0) ? (now - monitorState.lastWakeAt)
                                                             : MonitorPostWakeGraceSeconds;
  BOOL graceActive =
      (sinceClose < MonitorPostCloseGraceSeconds) || (sinceWake < MonitorPostWakeGraceSeconds);
...
void MonitorRun(const MonitorConfig *config, volatile sig_atomic_t *shouldRunFlag) {
  ...
  monitorState = (MonitorState){.savedBrightness = -1.0f,
                                .brightnessLowered = NO,
                                .lastAngle = -1,
                                .belowThresholdStreak = 0,
                                .lastFullCloseAt = 0.0,
